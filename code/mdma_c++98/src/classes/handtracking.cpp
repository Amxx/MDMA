...
cv::Point* HandTracking::list2arr(QList<QPoint> lst, int& n)
{
    ...
    for(QList<QPoint>::Iterator it = lst.begin() ; it != lst.end() ; ++it)
        result[n++] = cv::Point(it->x(), it->y());
...
void HandTracking::PosAreaCalcMasked(cv::Mat img_bin, double& area, QPoint& pos)
{
    ...
    for(std::vector<std::vector<cv::Point> >::iterator it = contours.begin() ; it != contours.end() ; ++it)
        contour_merged.insert(contour_merged.end(), it->begin(), it->end());
	cv::convexHull(cv::Mat(contour_merged), hull);
	area = cv::contourArea(hull);
	pos = QPoint(0, 0);
    //for(cv::Point p : hull)
    //	pos += QPoint(p.x, p.y);
    for(std::vector<cv::Point>::iterator it = hull.begin() ; it != hull.end() ; ++it)
        pos += QPoint(it->x, it->y);
	pos /= hull.size();

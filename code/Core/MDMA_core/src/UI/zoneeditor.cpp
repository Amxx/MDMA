	...
	evz(evz)
{
	...
	ui->lineEdit_x1->setText(QString::number(evz.pos[0].x()));
	ui->lineEdit_y1->setText(QString::number(evz.pos[0].y()));
	ui->lineEdit_x2->setText(QString::number(evz.pos[1].x()));
	ui->lineEdit_y2->setText(QString::number(evz.pos[1].y()));
	ui->comboBox_tab->setCurrentIndex(evz.tab);

	ui->tabWidget_type->setCurrentIndex(evz.type);

	ui->comboBox_axex_0->setCurrentIndex(MDMA::isMidi(evz.active[MDMA::EVENT_X])?evz.active[MDMA::EVENT_X]:MDMA::GOTO_TAB1);
	...
	ui->comboBox_axey_0->setCurrentIndex(MDMA::isMidi(evz.active[MDMA::EVENT_Y])?evz.active[MDMA::EVENT_Y]:MDMA::GOTO_TAB1);
...
void ZoneEditor::on_buttonBox_accepted()
{
	...
	evz.pos[0] = QPoint(ui->lineEdit_x1->text().toInt(), ui->lineEdit_y1->text().toInt());
	evz.pos[1] = QPoint(ui->lineEdit_x2->text().toInt(), ui->lineEdit_y2->text().toInt());
	...
	if(!MDMA::isMidi(evz.active[MDMA::EVENT_X])) evz.active[MDMA::EVENT_X] = MDMA::NOTHING;
	...
	if(!MDMA::isMidi(evz.active[MDMA::EVENT_Y])) evz.active[MDMA::EVENT_Y] = MDMA::NOTHING;
...
{
	ui->spinBox_axex_1->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_axex_2->setDisabled((MDMA::isMidi((MDMA::active) index) < 1)); // || evz.variable[0][0]);
	ui->spinBox_axex_3->setDisabled((MDMA::isMidi((MDMA::active) index) < 2)); // || evz.variable[0][1]);
}

void ZoneEditor::on_comboBox_axey_0_currentIndexChanged(int index)
{
	ui->spinBox_axey_1->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_axey_2->setDisabled((MDMA::isMidi((MDMA::active) index) < 1)); // || evz.variable[1][0]);
	ui->spinBox_axey_3->setDisabled((MDMA::isMidi((MDMA::active) index) < 2)); // || evz.variable[1][1]);
}
...
{
	ui->spinBox_enter_1->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_enter_2->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_enter_3->setDisabled(MDMA::isMidi((MDMA::active) index) < 2);
}

void ZoneEditor::on_comboBox_exit_currentIndexChanged(int index)
{
	ui->spinBox_exit_1->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_exit_2->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_exit_3->setDisabled(MDMA::isMidi((MDMA::active) index) < 2);
}

void ZoneEditor::on_comboBox_open_currentIndexChanged(int index)
{
	ui->spinBox_open_1->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_open_2->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_open_3->setDisabled(MDMA::isMidi((MDMA::active) index) < 2);
}

void ZoneEditor::on_comboBox_close_currentIndexChanged(int index)
{
	ui->spinBox_close_1->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_close_2->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_close_3->setDisabled(MDMA::isMidi((MDMA::active) index) < 2);
}

void ZoneEditor::on_comboBox_shock_currentIndexChanged(int index)
{
	ui->spinBox_shock_1->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_shock_2->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_shock_3->setDisabled(MDMA::isMidi((MDMA::active) index) < 2);
}


void ZoneEditor::on_comboBox_in_currentIndexChanged(int index)
{
	ui->spinBox_in_1->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_in_2->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_in_3->setDisabled(MDMA::isMidi((MDMA::active) index) < 2);
}

void ZoneEditor::on_comboBox_out_currentIndexChanged(int index)
{
	ui->spinBox_out_1->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_out_2->setDisabled(MDMA::isMidi((MDMA::active) index) < 1);
	ui->spinBox_out_3->setDisabled(MDMA::isMidi((MDMA::active) index) < 2);
}
...
void EditZone(Zone& evz, QWidget* parent)
{
	ZoneEditor(evz, parent).exec();
}
